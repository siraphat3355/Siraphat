...
byte mac[] = {  0xDE, 0xED, 0xBA, 0xFE, 0xFF, 0xEF };
...
{
  //TCNT3 = 65536 - (16000000.0 / 256.0 / (1 /0.01));
  PID_Control_Run();
  TCNT3 =  Timerz;
  /*
...
void loop()
{
  ...
  */
  //setpoint = mqttQueryString("Setpoint4_",mData).toInt();
...
void reconnect()
{
  ...
  //while (!client.connected())
  {
    ...
    if (client.connect("Board4"))
    {
      ...
      client.publish("Test1", "Board4 is connection");
    }
    else
    {
...
void Init_TIMER3_interuppt()
{
  ...
  TCCR3A = 0;
  TCCR3B = 0;
  TCNT3  = Timerz;           // preload timer 10ms
