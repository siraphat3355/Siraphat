...
byte mac[] = {  0xDE, 0xED, 0xCA, 0xFE, 0xFF, 0xEF };
IPAddress    ip(158, 108, 5, 3);
...
{
  currentPos = (readEncoder()) + (L3_Home * FactorPulseperCm);
...
void callback(char* topic, byte* payload, unsigned int length)
{
  ...
  if (String(topic) == "Test2")
  {
    Init_PID_Control() ;
    setpoint = mqttQueryString("Setpoint3_", mData).toInt();
    pulse = setpoint - currentPos;
    mMode = 0;
  }
  if (String(topic) == "Test3")
  {
    Vv = mqttQueryString("Velocity3_", mData).toInt();
  }
  //Serial.println();
...
void reconnect()
{
  ...
  {
    if (client.connect("Board3"))
    {
      client.subscribe("Test2");
      client.publish("Test1", "Board3 is connection");
    }
    else
    {
      delay(10);
